...
+!drive
    ...
    : ~>>allowedspeed(_) <-
        // ternary operator: accelerate with 70% if we are driving <= 80, otherwise try our luck and accelerate 10%
    	CurrentSpeed <= 80 ? vehicle/accelerate(0.7) : vehicle/accelerate(0.1);
    	!drive

	// if we know the allowed speed, try to accelerate
	: >>allowedspeed(S) && CurrentSpeed < S <-
