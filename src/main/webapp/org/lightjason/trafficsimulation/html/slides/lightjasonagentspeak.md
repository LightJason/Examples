...
* Consider every execution path! For the method `phaseduration` this would be
  1. If the phase duration is less than one second, the plan always fails ($\rightarrow$ early exit).
...
* They can be defined as 
  ...
  ($\rightarrow$ see example on the next slide)
...
will execute the two plans immediately in the given order, i.e. `+!phaseduration(90)` $\rightarrow$ `+!phaseduration(60)`.
...
* Write a plan `+!changelight` which changes the traffic light to the next logical state (depending on the current), i.e. `light(red)` $\rightarrow$ `light(redyellow)` $\rightarrow$ `light(green)` $\rightarrow$ `light(yellow)` $\rightarrow$ `light(red)` $\rightarrow$ ... each time the plan gets triggered.
* Hints:
